                ...
                getDefaultState() {
                    ...
                    return {
                        ...
                        blocks: [
                            ...
                            {
                                id: this.generateId('b'), type: 'text', title: "双排卡片区块", isVisible: true, isExpanded: false, settings: { layout: 'dual', masonryEnabled: false }, cards: [
                                    { id: this.generateId('c'), icon: '', title: "这是双排卡片 1", content: "PC端可以用ctrl+Z/Y/B/I分别实现撤回/重做/字体粗体/斜体功能哦！", opacity: 1.0, followGlobalOpacity: true, textShadowEnabled: false, titleColor: null, titleFontSize: null, contentFontSize: null },
                                    { id: this.generateId('c'), icon: '', title: "这是双排卡片 2", content: "双击这里进行编辑", opacity: 1.0, followGlobalOpacity: true, textShadowEnabled: false, titleColor: null, titleFontSize: null, contentFontSize: null }
                                ]
                            },
                    ...
                    panel.addEventListener('click', e => {
                        ...
                        const actions = {
                            ...
                            '#clear-page-bg-btn': () => {
                                ...
                                this.updateState('pageStyles.pageBgImageDataUrl', null, true, '清除页面背景图');
                ...
                renderLayerPanel() {
                    ...
                    html += this.state.blocks.map(b => {
                        ...
                        const cardsListHTML = isExpandable ? `<ul class="card-layer-list" data-block-id="${b.id}">
                            ${b.cards.map(c => `<li class="card-layer-item ${cardId === c.id ? 'selected' : ''}" data-card-id="${c.id}">${this.escapeHTML(c.title) || '无标题卡片'}</li>`).join('')}
                        </ul>` : '';

                        const quickAddBtn = hasQuickAdd ? `<button class="btn-icon add-card-to-block-btn" title="添加卡片/图片"><span class="iconify" data-icon="mdi:plus"></span></button>` : '';
								...
								<hr class="separator">
                                    <div class="tab-group-wrapper">
                                        ...
                                        <div id="page-bg-solid" class="tab-content">
                                            ...
                                            <div class="form-group"><label>背景图 (可选):</label><div class="input-group simple"><input type="file" id="page-bg-upload" accept="image/*" style="width:100%"><button id="clear-page-bg-btn" class="btn btn-default btn-small">清除</button></div></div>
                    ...
                    if (mainKey === 'blocks') {
                        ...
                        if (updatedProperty === 'cards') {
                            ...
                            if (cardProp === 'title') {
                                ...
                                if (cardLayerItem) cardLayerItem.textContent = card.title || '无标题卡片';
                            }
                            this.renderPreviewBlockById(block.id);
                            return;
                        }
                ...
                exportConfig(isTemplate = false) {
                    ...
                    if (isTemplate) {
                        ...
                        stateToSave.blocks.forEach(block => {
                            if (block.type === 'text' || block.type === 'image') {
                                block.title = block.type === 'text' ? "文本区块" : "图片区块";
